...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $descricao_status = isset($descricao[$status_sugestao]) ? $descricao[$status_sugestao] : '';

    // Preparar a consulta SQL
   // Preparar a consulta SQL
$sql = "INSERT INTO `registros de sugestoes` (nome, email, Sugestao, arquivo_nome2, Cod_Sugestao, status_sugestao, descricao, dataregistroSugestao) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if ($link_arquivo) {
        ...
        if ($stmt) {
            $stmt->bind_param("ssssssss", $nome, $email, $Sugestao, $link_arquivo, $Cod_Sugestao, $status_sugestao, $descricao_status, $dataregistroSugestao);
        } else {
        ...
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            $stmt->bind_param("sssssss", $nome, $email, $Sugestao, $Cod_Sugestao, $status_sugestao, $descricao_status, $dataregistroSugestao);
        } else {
            die("Erro na preparação da consulta: " . $conn->error);
        }
    }
