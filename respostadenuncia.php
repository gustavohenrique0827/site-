...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $descricao_status = isset($descricao[$status_denuncia]) ? $descricao[$status_denuncia] : '';

    // Preparar a consulta SQL
    $sql = "INSERT INTO `registros` (nome, email, Denuncia_interna, arquivo_nome3, Cod_Denuncia, status_denuncia, descricao, dataregistro) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if ($link_arquivo) {
        ...
        if ($stmt) {
            $stmt->bind_param("ssssssss", $nome, $email, $denuncia, $link_arquivo, $Cod_Denuncia, $status_denuncia, $descricao_status, $datadenuncia);
        } else {
        ...
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            $stmt->bind_param("sssssss", $nome, $email, $denuncia, $Cod_Denuncia, $status_denuncia, $descricao_status, $datadenuncia);
        } else {
            die("Erro na preparação da consulta: " . $conn->error);
        }
    }
