...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $denuncia = trim($_POST['Denuncia_interna']);
    $anonimo = isset($_POST['anonimo']) && $_POST['anonimo'] === "sim" ? true : false;
    $datadenuncia = date('Y-m-d H:i:s');
    ...
    if (!empty($_FILES['arquivo']['name'])) {
        ...
        $arquivo_nome = $nome . "_Denúncia_" . pathinfo($arquivo['name'], PATHINFO_EXTENSION);
    ...
    $status_denuncia_interna = "Recebida";
    ...
    $descricao_status = isset($descricao[$status_denuncia_interna]) ? $descricao[$status_denuncia_interna] : '';

    // Preparar a consulta SQL
    $sql = "INSERT INTO `registros internos` (nome, email, Denuncia_interna, arquivo_nome3, Cod_Denuncia, status_denuncia_interna, descricao, dataregistro_interno) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if ($link_arquivo) {
        ...
        if ($stmt) {
            $stmt->bind_param("ssssssss", $nome, $email, $denuncia, $link_arquivo, $Cod_Denuncia, $status_denuncia_interna, $descricao_status, $datadenuncia);
        } else {
            die("Erro na preparação da consulta: " . $conn->error);
        }
    } else {
        // Sem arquivo (remover `arquivo_nome3`)
        $sql = "INSERT INTO `registros internos` (nome, email, Denuncia_interna, Cod_Denuncia, status_denuncia_interna, descricao, dataregistro_interno) 
                VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            $stmt->bind_param("sssssss", $nome, $email, $denuncia, $Cod_Denuncia, $status_denuncia_interna, $descricao_status, $datadenuncia);
        } else {
            die("Erro na preparação da consulta: " . $conn->error);
        }
    }
...
?>
    ...
    <div class="chat-button" id="chat-button" onclick="toggleChat()">
        <img src="imagens_botoes/chat-bot.png" alt="Chat Icon" class="img-fluid">
    </div>
