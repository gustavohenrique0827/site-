...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $descricao_status = isset($descricao[$status_reclamacao]) ? $descricao[$status_reclamacao] : '';
    ...
    if ($link_arquivo) {
        ...
        if ($stmt) {
            $stmt->bind_param("ssssssss", $nome, $email, $Reclamacao, $link_arquivo, $Cod_Reclamacao, $status_reclamacao, $descricao_status, $datareclamacao);
        } else {
        ...
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            $stmt->bind_param("sssssss", $nome, $email, $Reclamacao, $Cod_Reclamacao, $status_reclamacao, $descricao_status, $datareclamacao);
        } else {
            die("Erro na preparação da consulta: " . $conn->error);
        }
    }
    ...
    if ($stmt && $stmt->execute()) {
        ...
        echo "<div class='alert alert-danger'>Erro ao enviar Reclamação: " . ($stmt ? $stmt->error : "Erro na execução da consulta") . "</div>";
    }
